...
class claimsApi {
  ...
  Future<List<ClaimDetailResponse>> get_claims_details(
      String notificationId, String companyCode, String token) async {
    var response = await http.get(
        Uri.parse(AppUrl.claimsDetails +
            "notificationId=" +
            notificationId +
            "&companyCode=" +
            companyCode),
    ...
    if (response.statusCode == 200) {
      final claimsDetailsData =
          json.decode(response.body)['claimDetailBeanList'];
  ...
  Future<bool> deleteCarsSurvey(
      String carId, String userId, String token) async {
    var response = await http.delete(
        Uri.parse(AppUrl.deleteCarsSurvey + carId + "&userId=" + userId),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json',
          'Authorization': 'Bearer $token',
  ...
  Future<String> insertLossCar(
      String notification, String userId, String token) async {
    var response = await http.post(
      Uri.parse(AppUrl.insertLossCar),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      ...
      body: jsonEncode(
          <String, String>{'userId': userId, 'notification': notification}),
    );
  ...
  Future<void> insertCarsSurvey(
      String carId, String userId, String token) async {
    var response = await http.post(
      Uri.parse(AppUrl.insertCarsSurvey),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      },
      body: jsonEncode(<String, String>{'userId': userId, 'carId': carId}),
    );
    if (response.statusCode == 201 || response.statusCode == 200) {
    } else {
      throw Exception('Failed to  get insert loass car');
